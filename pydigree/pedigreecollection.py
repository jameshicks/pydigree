...
class PedigreeCollection(MutableMapping):
    ...
    def additive_relationship_matrix(self):
        ...
        """
        return block_diag([x.additive_relationship_matrix() for x in sorted(self,key=lambda x: x.label)],format='bsr')
    def dominance_relationship_matrix(self):
        ...
        """
        return block_diag([x.dominance_relationship_matrix() for x in sorted(self,key=lambda x: x.label)],format='bsr')
    def mitochondrial_relationship_matrix(self):
        ...
        """
        return block_diag([x.mitochondrial_relationship_matrix() for x in sorted(self,key=lambda x: x.label)],format='bsr')
