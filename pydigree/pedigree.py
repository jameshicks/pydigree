...
class Pedigree(Population):
    ...
    def additive_relationship_matrix(self, ids=None):
        ...
        Important: if not given, the rows/columns are all individuals in the pedigree, sorted by id.
        If you're not sure about this, try sorted(x.id for x in ped) to see the ordering.

        Returns: a numpy matrix
        """
        if not ids: ids = sorted(x.id for x in self)
        mat = []
        for a in ids:
            row = []
            for b in ids:
